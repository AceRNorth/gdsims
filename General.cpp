#include "headGen.h"
	...
	/*---------input parameters---------------------*/
		...
		cin>>pa.gamma;//rate of r2 allele formation from W/D meiosis
	...
	void initiate(void){
		...
		{       
			...
			pp.sqy=0;
			pp.CentSq=0;
			if(distance(pa.U,pp.x,pp.y,pa.U/2.0,pa.U/2.0)<pa.CentRad)pp.CentSq=1;
...
//		cout<<"os1  "<<to.JTot<<"   "<<to.MTot<<"    "<<to.VTot<<"   "<<to.FTot<<endl;
	//	CheckCounts(0,'o');
		JuvGetOlder();
		AdultsDie();
		VirginsMate();
		...
		for(int pat=0;pat<Site.size();pat++)
			...
			for(int i=0;i<NumGen;i++)
					...
					if(v[i]>0)
						...
						for(int j=0;j<NumGen;j++)Site[pat].F[i][j]+=randomCounts[j];
						to.F[i]+=v[i];
	...
	void LayEggs (void){
		...
		std::vector<int> randomCounts;
		for(int i=0;i<TL;i++)larv[i]=pa.LarvProbs[i];
		int* aww;
		for(int pat=0;pat<Site.size();pat++)
				{
				...
				for(int i=0;i<NumGen;i++)
					...
					for(int j=0;j<NumGen;j++)
						...
						for(int k=0;k<NumGen;k++)
							...
							for(int a=0;a<TL;a++) Site[pat].J[k][a]+=randomCounts[a];
...
std::vector<int> random_Multinomial(int N,const std::vector<double>& probs)
{
	...
	for (int i = 0; i < num_outcomes; ++i) sum_p+=probs[i];
	int Nused=N;
...
void CheckCounts(int TT,char ref){
	...
	for(int pat=0;pat<Site.size();pat++)
		...
		for(int i=0;i<NumGen;i++)
		{
			for(int j=0;j<NumGen;j++)if(Site[pat].F[i][j]<0)
			{cout<<TT<<"   "<<ref<<" F neg  "<<i<<"   "<<j<<"  "<<Site[pat].F[i][j]<<endl;exit(1);};
		};

		for(int i=0;i<NumGen;i++)totF+=accumulate(Site[pat].F[i],Site[pat].F[i]+NumGen,0);
		totM+= accumulate(Site[pat].M,Site[pat].M+NumGen,0);
		totV+= accumulate(Site[pat].V,Site[pat].V+NumGen,0);
		for(int i=0;i<NumGen;i++)
			{
				for(int a=0;a<TL;a++) totJ+=Site[pat].J[i][a];
			};
		};
	...
	if(totF!=to.FTot){cout<<TT<<"   "<<ref<<" F count error  "<<totF<<"   "<<totFB<<"   "<<to.FTot<<endl;exit(1);};
	if(totM!=to.MTot){cout<<TT<<"   "<<ref<<" M count error  "<<totM<<"   "<<totMB<<"   "<<to.MTot<<endl;exit(1);};
	if(totV!=to.VTot){cout<<TT<<"   "<<ref<<" V count error  "<<totV<<"   "<<totVB<<"   "<<to.VTot<<endl;exit(1);};
	if(totJ!=to.JTot){cout<<TT<<"   "<<ref<<" J count error  "<<totJ<<"   "<<totJB<<"   "<<to.JTot<<endl;exit(1);};
	if(totFB!=to.FTot){cout<<TT<<"   "<<ref<<" F count errorB  "<<totFB<<"   "<<to.FTot<<endl;exit(1);};
	if(totMB!=to.MTot){cout<<TT<<"   "<<ref<<" M count errorB  "<<totMB<<"   "<<to.MTot<<endl;exit(1);};
	if(totVB!=to.VTot){cout<<TT<<"   "<<ref<<" V count errorB  "<<totVB<<"   "<<to.VTot<<endl;exit(1);};
	if(totJB!=to.JTot){cout<<TT<<"   "<<ref<<" J count errorB  "<<totJB<<"   "<<to.JTot<<endl;exit(1);};
	return;};
